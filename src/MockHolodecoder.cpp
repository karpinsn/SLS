...
MockHolodecoder::MockHolodecoder(OpenGLWidget* glContext)
{
...
void MockHolodecoder::initShaders(void)
{
	//	Create the shaders
	...
	OGLStatus::logOGLErrors("MockHolodecoder - initShaders()");
}

void MockHolodecoder::_initTextures(GLuint width, GLuint height)
{
	Logger::logDebug("MockHolodecoder - initTextures(): Creating textures for phase map and normal map");
	...
	OGLStatus::logOGLErrors("MockHolodecoder - initTextures()");
}

void MockHolodecoder::draw(void)
{
	m_imageProcessor.bind();
	{
		...
		if(once)
		{
			...
			//	Save out the normal map
			glReadBuffer(m_normalMapAttachPoint);
			io.saveBoundBuffer("/Users/karpinsn/Downloads/NormalMap.png", 512, 512);
		}
	...
	OGLStatus::logOGLErrors("MockHolodecoder - draw()");
}

void MockHolodecoder::resize(int width, int height)
{
	m_camera->reshape(width, height);
	gluPerspective(45.0, 1.0, .00001, 10.0);
}

void MockHolodecoder::cameraSelectMode(int mode)
{
	m_camera->setMode(mode);
}

void MockHolodecoder::mousePressEvent(int mouseX, int mouseY)
{
	m_camera->mousePressed(mouseX, mouseY);
}

void MockHolodecoder::mouseMoveEvent(int mouseX, int mouseY)
{
	m_camera->mouseMotion(mouseX, mouseY);
}

void MockHolodecoder::setBackHoloBuffer(IplImage* image)
{
...
void MockHolodecoder::swapBuffers(void)
{
...
void MockHolodecoder::_initLighting(void)
{
