...
void main(void)
{
  ...
  float phaseReference = (newVertex.x / pixelWidth) * ((2.0*pi*cos(theta))/P);
  ...
  for(int i=0; i<8; ++i)
  {
    ...
    float neighborPhaseReference = ((newVertex.x + offset[i].x) / pixelWidth) * ((2.0*pi*cos(theta))/P);
    ...
    float nextNeighborPhaseReference = ((newVertex.x + offset[i+1].x) / pixelWidth) * ((2.0*pi*cos(theta))/P);
